...
define('blame', [], function () {
  ...
  typedef('Num', simple_contract(ground('number')));
  typedef('Str', simple_contract(ground('string')));
  typedef('Bool', simple_contract(ground('boolean')));
  ...
  // * => *
  typedef('Fun', simple_contract(isFunction));
  ...
  // TODO: Add support for functions accepting 0 arguments
  function TFun() {
    ...
    var new_contract = compose.call(this, blame.Fun.contract, function(func, label, isLast, context) {
      var negated = '~' + label;
      var that = this;
      ...
      return function() {
        ...
        // TODO: Test for correct arity here

        fun_args.forEach(function (arg, index) {
          if (index < domain.length) {
            wrapped_args.push(domain[index].contract.call(that, arg, negated, false, context));
          }
          else {
        ...
        return range.contract.call(that, func.apply(undefined, wrapped_args), label, true, context);
      };
    });
  ...
  var Forall = function(X, type) {
    ...
    var name = 'forall ' + Xs.join(' ') + ', ' + type.name;


    var that = this;
    var new_contract = function(value, label, isLast) {
      return (function() {
        ...
        (function(obj) {
          ...
          Xs.forEach(function (x) {
            var token = Object.create(Object.prototype);
            Object.freeze(token);
            tokens[x] = token;
          });
          var storage = Object.create(Object.prototype);
          ...
          obj.keep = function(name, value) {
            if (tokens[name]) {
              if (storage[name]) {
                ...
                console.log('got', name, storage[name]);
        ...
        return type.contract.call(that, value, label, isLast, context);
      }());
    };

    Type.call(this, name, new_contract);
  };
  ...
  var Tyvar = function(name) {
    ...
    var ty_contract = function(value, label, isLast, context) {
      console.log(context);
      if (!(this instanceof Forall)) {
        // TODO: Figure what to throw later
        throw new Error('Tyvar not inside Forall');
      }
      // Attempt to unseal the value:

      if(isLast) {
        var result = context.have(name, value);
        if (!result) {
          throw new Error(label);
        }
      ...
      var token = context.keep(name, value);
      if (!token) {
        throw new Error(label);
      }
      return token;
