...
define('blame', [], function () {
  'use strict';
  var blame = Object.create(null),
    Num, Bool, Str, Und, Any, Forall, fun, arr, tyvar, forall;
  ...
  function Fun(domain, optional, repeated, range) {
    ...
    if (domain.length === 0 && optional.length === 0 && !repeated) {
      descs.push('()');
    }
  ...
  function substitute_tyvar(target, id, new_type) {
    var tyvar = String(id),
      new_domain, new_optional, new_repeated, new_range;
    ...
    if (target instanceof Forall) {
      ...
      return new Forall(target.tyvar, substitute_tyvar(target.type, tyvar, new_type));
    }
    ...
    if ((A instanceof Fun) && (B instanceof Fun) && A.domain.length === B.domain.length && A.optional.length === B.optional.length && !!A.repeated === !!B.repeated) {
      ...
      return new Proxy(value, {
        apply: function (target, thisValue, args) {
          ...
          for (i = 0; i < A.domain.length; i++) {
            wrapped_args.push(wrap(args[i], q, p, B.domain[i], A.domain[i]));
          }
          for (j = 0; j < A.optional.length && (i + j) < args.length; j++) {
            wrapped_args.push(wrap(args[i + j], q, p, B.optional[i], A.optional[i]));
          }
          for (k = 0; (i + j + k) < args.length; k++) {
        ...
        get: function (target, name, receiver) {
          ...
          var ARRAY_API = {
            ...
            forEach: function (T) {
              ...
              return func(fun_type, Und);
            },
            indexOf: function (T) {
              return func(T, Num, Num);
            },
            join: function () {
              return new Fun([], [Str], null, Str);
            },
            ...
            map: function (T) {
              ...
              return forall('Z', func(fun_type, arr(tyvar('Z'))));
            },
            pop: function (T) {
              return func(T);
            },
            ...
            reduce: function (T) {
              var fun_type = func(tyvar('Z'), T, Num, arr(T), tyvar('Z'));
              return forall('Z', func(fun_type, tyvar('Z'), tyvar('Z')));
            },
