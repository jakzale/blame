...
define('blame', [], function () {
  ...
  function TFun() {
    ...
    function contract(fun, label, context) {
      var negated, handler, arg_context;

      arg_context = new Context(true, context);

      negated = '~' + label;

      function wrap_argument(context) {
        return function (arg, index) {
          return domain[index].contract.call(undefined, arg, negated, arg_context);
        };
      }
      ...
      handler = {
        apply: function (target, thisArg, args) {
          ...
          wrapped_args = map(args, wrap_argument(context));
          result = target.apply(thisArg, wrapped_args);

          return range.contract.call(undefined, result, label, context);
        },
        //construct: function (target, args) {
        //  var wrapped_args, instance;
        ...
        //  return range.contract.call(undefined, instance, label, context, is_unseal);
        //}
      };
  ...
  function Forall(id, type) {
    ...
    this.contract = function (value, label, context) {
      var my_context = new Context(false, context);
      ...
      var handler = {
        apply: function (target, thisArg, args) {
          // TODO: Here generate the seal storage
            ...
            wrapped_fun = type.contract.call(undefined, target, label, my_context);
  ...
  function Tyvar(id) {
    ...
    this.contract = function(value, label, context) {
      ...
      // TODO: Implement the seal storage here
      var is_unseal = context.is_last;
  ...
  function TArr(type) {
    ...
    function contract(value, label, context) {
      ...
      var handler = {
        ...
        get: function (target, name, receiver) {
          var index,
            get_context = new Context(false, context);
          ...
          if (name === index.toString() && index !== (-1 >>> 0)) {
            ...
            return type.contract(target[name], label, get_context);

          }

          if (ARRAY_API.hasOwnProperty(name)) {
            if (ARRAY_API[name]) {
              return ARRAY_API[name].contract(target[name], label, get_context);
            }
        ...
        set: function (target, name, value, receiver) {
          var index, negated,
            set_context = new Context(true, context);
          ...
          if (name === index.toString() && index !== (-1 >>> 0)) {
            ...
            target[name] = type.contract(value, negated, set_context);
          }
        }
