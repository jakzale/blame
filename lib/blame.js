...
define('blame', [], function () {
  ...
  Label.prototype.msg = function(message) {
    return '{' + (this.status ? 'positive ' : 'negative ') + this.label + '} ' + (message || '');
  };
  ...
  function wrap(value, p, q, A, B) {
    ...
    if ((A instanceof BaseType) && (B instanceof BaseType) && A === B) {
      return apply_contract(value, A.contract, p.msg(value + ' is not of type ' + A.description));
    }

    if ((A instanceof Fun) && (B instanceof Fun) && A.domain.length === B.domain.length) {
      // Check if enclosed value is  function
      apply_contract(value, is_function, p.msg(value + ' is not a function'));
    ...
    if ((A instanceof Arr) && (B instanceof Arr)) {
      apply_contract(value, is_array, p.msg(value + ' is not an array'));

      // Treatment of the arrays
      // Storing a value is similar to invoking a function Type -> ()
      // Returning a value is similar to invoking a function Number -> Type, where Type is the type of the array
      return new Proxy(value, {
        get: function (target, name, receiver) {
          unused(receiver);

          // Not testing length at the moment
          if (name === 'length') {
