...
define('blame', [], function () {
  ...
  // Creating the panicking token proxy

  // TODO: Add Panicking proxy later
  //function token(tyvar) {
  ...
  // TODO: The Current implementation of is_unsealed is wrong.
  // the variable, which passed through the binder for X has the type * within the function,
  // it can safely go through function * -> *.
  // The only thing is that if the element is Any for the top forall type,
  // it has to check for sealed values.
  //
  // Actually, nu_tamper check on the range on Any, should happen for each function which takes any
  // tyvar arguments and returns a value of type Any.

  function is_unsealed(value) {
    return !(value instanceof Token);
  }

  // TODO: This needs to be changed to include nu_tamper
  typedef('Any', simple_contract(is_unsealed));
  ...
  // TODO: Figure out better error messages
  function unseal(value, tyvar, label, context) {
    if (is_unsealed(value)) {
