...
define('blame', [], function () {
  'use strict';
  var blame, count, Num, Bool, Str, Und, Any, Forall, fun, arr, tyvar, forall, obj, sum, wrap, ARRAY_API;
  ...
  ARRAY_API.splice = {
    get: function (A) {
      ...
      return fun([Num, Num], null, T, A);
    }
  };

  ARRAY_API.toLocaleString = {
    get: function () {
      return func(Str);
    }
  };
  ...
  function wrap_array(value, p, q, A, B) {
    return new Proxy(value, {
      get: function (target, name, receiver) {
        ...
        var gen, A_type, B_type;


        // Fix, because there is Object.prototype.toString
        if (ARRAY_API[name] && ARRAY_API[name].get) {
          gen = ARRAY_API[name].get;
      ...
      set: function (target, name, val, receiver) {
        ...
        if (ARRAY_API[name] && ARRAY_API[name].set) {
          gen = ARRAY_API[name].set;
          A_type = gen(A);
          B_type = gen(B);
