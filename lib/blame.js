...
define('blame', [], function () {
  ...
  function compose_contracts(g, h) {
    return function (value, label) {
      return h(g(value, label), label);
    };
  }

  function wrap_arguments(args, domain, label) {
    var wrapped_arguments = [];
    args.forEach(function (arg, index) {
      if (index < domain.length) {
        wrapped_arguments.push(domain[index].contract(arg, label));
      } else {
      ...
      var negated = '~' + label,
        handler = {
          'apply': function (target, thisArg, args) {
            var wrapped_args = wrap_arguments(args, domain, negated),
              result = target.apply(thisArg, wrapped_args);

            return range.contract(result, label);
          },
          'construct': function (target, args) {
            ...
            return range.contract(instance, label);
          }
        };
