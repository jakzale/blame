...
define('blame', [], function () {
  ...
  blame.tFun = function () {
    ...
    description = map(args, function (arg, index) {
      ...
      var negated = '~' + label,
        ...
        handler = {
          apply: function (target, thisArg, args) {
            var wrapped_args, result, context, key, my_type_variables;
            // TODO: Generate a new instance of a type with seals attached to it.

            context = {};

            if (type_variables.has(that)) {
              my_type_variables = type_variables.get(that);

              // Check if there are any type variables define on the type
            ...
            wrapped_args = wrap_arguments(args, domain, negated, context);
            result = target.apply(thisArg, wrapped_args);

            return range.contract.call(context, result, label, true);
          },
  ...
  //  new_type = new Type(description, type.contract);
